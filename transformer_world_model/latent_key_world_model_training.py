...
from jaxued.environments.maze import make_level_w_key_generator
...
def main(config=None, project="JAXUED_TEST"):
    ...
    def train_step(runner_state, _):
        ...
        def loss_fn(params):
            (z_dist, pred_next_z_dist), (recon_obs, pred_reward, pred_done), (l_recon, l_pred, l_dyn) = train_state.apply_fn(
                params, 
                rng_wm, 
                obs, 
                actions,
            ...
            if config['gen_first_obs']:
                mode_accuracy = (pred_next_z_dist.mode() == z_dist.mode()).mean()
                conditioned_mode_accuracy = (pred_next_z_dist.mode() == z_dist.mode())[100:].mean()
            else:
            ...
            loss = l_recon + l_pred + l_dyn - config['entropy_coeff'] * entropy
            return loss, ((l_recon, l_pred, l_dyn, entropy), (mode_accuracy, conditioned_mode_accuracy))
        
        grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
        (loss, (losses, accuracies)), grads = grad_fn(train_state.params)
    ...
    def train_and_eval(runner_state, _):
        ...
        (eval_loss, (eval_losses, eval_accuracies)), (latent_eval_loss, (latent_eval_losses, latent_eval_accuracies)) = eval(_rng, train_state)
...
def config_main(config: DictConfig):
    ...
    config['group'] = "RoPE Transformer Latent World Model"
